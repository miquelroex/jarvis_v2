...
* [x] **Saludo Contextual al Arrancar**: en vez de un saludo fijo, Jarvis abre según la hora, el clima, tu agenda y cómo acabó la última sesión. *"Buenos días, señor. 7:42, lloviznando. Tiene una reunión a las 9 y dejó el build en rojo anoche."* ✅ core/startup.py: generate_startup_greeting ahora es CONCISO y contextual por defecto — hora + clima (reutiliza morning_briefing) + recordatorios del día + sólo alertas críticas (dispositivos desconocidos, RAM alta); build_contextual_greeting puro. El parte técnico verboso de antes (memoria/servicios/red) queda detrás de JARVIS_STARTUP_VERBOSE=true. 12 tests, 96% mutation score. (Pendiente opcional: "cómo acabó la última sesión" —build en rojo— requiere guardar estado al cerrar).
...
* [x] **Webcam en tiempo real**: seguimiento, reconocimiento facial (saber quién entra), detección de presencia. *"Bienvenido, señor."* / *"Hay alguien más en la sala."* ✅ Detección de presencia: core/presence.py (servicio #30, off por defecto): cuenta personas con Gemini Vision + máquina de estados PURA con anti-parpadeo (PresenceMonitor → arrival/departure/companion/alone). 21 tests, 68% mutation. ✅ Reconocimiento de identidad LOCAL y privado: core/face_id.py (OpenCV LBPH, dependencia opcional perezosa `pip install opencv-contrib-python`): enrolas tu cara ("recuérdame, soy el señor") y distingue quién aparece — la llegada se personaliza ("Bienvenido, señor" vs "detecto a un desconocido"); resolve_identity/add_label/greeting puros, OpenCV aislado y degradando con gracia si no está instalado (CI a salvo). Voz "recuérdame mi cara", "¿quién soy?". 26 tests, 48% mutation (resto glue OpenCV). Las caras nunca salen del equipo. (Seguimiento/tracking continuo: opcional a futuro.)
* [x] **Memoria visual**: recuerda objetos/escenas vistas. *"¿Dónde dejé las llaves?" → "Sobre la mesa, señor, hace veinte minutos."* ✅ core/visual_memory.py (servicio #31, daemon off por defecto): observa la escena por webcam (a demanda "observa la escena" o periódicamente), reconoce objetos y dónde están con Gemini Vision y lo registra con su hora (logs/visual_memory.jsonl); parse_scene / find_object (por solapamiento de tokens) / relative_time / format_answer / summarize_recent puros; captura y visión aisladas. Voz "¿dónde dejé las llaves?", "observa la escena", "¿qué has visto?". 23 tests, 71% mutation (resto I/O aislado). Cierra el bloque de visión (presencia + identidad + memoria visual).

**🎙️ Voz verdaderamente full-duplex** (hoy es turno-a-turno)
* [~] **Streaming bidireccional con barge-in real**: Jarvis escucha mientras habla, le interrumpes con naturalidad, latencia mínima, sin botones ni esperas. Conversación de verdad. ✅ Rebanada 1 — barge-in robusto: core/barge_in.py extrae la lógica que vivía enterrada en el hilo de audio (tools/voice._vad_monitor_thread) a una máquina de estados PURA y testeada (calibración con rechazo de eco, umbral dinámico, disparo tras N frames). ✅ Rebanada 2 — handoff sin perder el turno: el VAD señala el barge-in (signal_barge_in) y la siguiente escucha lo consume (consume_barge_in) para capturar de inmediato, sin recalibrar y con pausa corta (capture_mode), enganchado en main.listen_for_next_command — así no se pierde el inicio de tu frase al interrumpir. 20 tests, 100% mutation score. (Pendiente, requiere micrófono para iterar: STT en streaming continuo de baja latencia.)
...
* [x] **APIs de datos en vivo (legal)**: noticias, finanzas (bolsa/cripto), vuelos, tráfico, sismos, clima detallado — con webhooks/streams en lugar de consultas puntuales. *"Señor, la acción que vigilaba acaba de caer un 4%."* ✅ core/live_data.py: APIs públicas SIN clave — **bolsa (Stooq)**, cripto (CoinGecko), sismos (USGS), tech-news (Hacker News); parsers puros + HTTP aislado; expuestas por voz ("precio de apple", "cómo va la bolsa", "precio del bitcoin", "últimos terremotos", "noticias de tecnología") y como fuente del Motor de Fusión con relevancia por tema. Los "webhooks/streams proactivos" se cubren con la Vigilancia del Mundo (#29), que ahora también vigila **acciones** y avisa al moverse ("la acción que vigilaba acaba de caer un X%"). live_data 33 tests / 87% mutation, world_watch +4 tests de bolsa. (Vuelos/tráfico: descartados — requieren claves/scraping; el patrón queda listo para añadir cualquier fuente nueva en una función.)
...
* [x] **Vigilancia de temas en internet**: como watchpost pero apuntando al mundo — vigila temas/personas/mercados y avisa de cambios. *"Novedades sobre el tema que seguía, señor."* ✅ core/world_watch.py (Vigilancia Proactiva del Mundo, servicio #29): registras qué seguir por voz ("vigila el bitcoin", "avísame de terremotos") y un daemon sondea los datos en vivo (live_data) avisando SIN preguntar ante un cambio relevante (variación cripto sobre umbral, sismo nuevo sobre magnitud); parseo (con casado por palabra para evitar 'sol'/'ada' en otras palabras), variación, umbrales y frases puros; sondeo aislado, alerta por voz + banner HUD 👁. 33 tests, 67% mutation score. Cubre también "Monitorización proactiva del mundo".
...
* [x] **Autenticación por voz/cara** (S): usar el reconocimiento facial (core/face_id) como candado para acciones sensibles. *"Identidad confirmada, señor. Acceso concedido."* / *"No le reconozco. Acceso denegado."* ✅ core/access_control.py: is_sensitive (clasifica comandos destructivos), decide_access (política pura: autorizado/desconocido/sin-cámara con fail-open configurable) y access_phrase puros y testeados; verificación facial aislada (face_id.identify). Enganchado al inicio de handle_fast_command: bloquea la acción sensible si no se autoriza. Off por defecto; voz "activa el candado". 22 tests, 62% mutation (resto verificación aislada). Hace ÚTIL el reconocimiento facial.

**🛡️ Stark paranoico (defensa del propio sistema)**
* [x] **Contra-intrusión del equipo** (S): vigila logins fallidos, procesos nuevos sospechosos, cambios en ficheros críticos y conexiones salientes extrañas, y avisa. *"Señor, alguien intenta acceder. He sellado el sistema."* ✅ core/intrusion.py (servicio #33, off por defecto): find_suspicious (procesos de pentest/credenciales: mimikatz, psexec, ncat…), new_suspicious (sólo los aparecidos desde el sondeo anterior), failed_login_spike, detect_events y build_scan_report puros y testeados; recolección aislada (psutil para procesos/conexiones externas vía is_private_ip, wevtutil para logins fallidos 4625 best-effort); daemon con anti-repetición. Defensivo y local (solo observa tu equipo). Voz "¿estamos seguros?" / "busca intrusos". 23 tests, 58% mutation (resto recolectores aislados).
...
* [x] **Diagnóstico con guía de reparación** (S): además del informe de daños, te guía paso a paso a resolver lo que falla. *"Propulsor izquierdo al 40%. Le guío: primero…"* ✅ core/repair_guide.py: detect_problems (sobre el snapshot del world_model + disco + tests: RAM alta, disco lleno, servicio caído, amenaza, tests en rojo, repo sucio, por gravedad) y una base de conocimiento problema→pasos; format_guide/build_diagnosis puros. Voz "diagnostícame" / "¿cómo lo arreglo?". 17 tests, 85% mutation score.
...
* [x] **Simulaciones y cálculos al vuelo** (S): ejecuta dry-runs/benchmarks y da probabilidades. *"Ejecutando simulación… probabilidad de éxito del despliegue: 91%."* ✅ core/simulation.py: ante "simula el despliegue", corre comprobaciones reales (smoke tests, RAM, amenaza, repo limpio) ponderadas y calcula una probabilidad de éxito; success_probability/verdict/format_simulation puros; comprobaciones aisladas. A diferencia de threat_assessment (riesgo del contexto), esto CORRE checks concretos. Voz "simula &lt;acción&gt;" / "¿qué probabilidad hay de &lt;x&gt;?". 14 tests, 93% mutation score.
...
* [x] **Aprendizaje y evolución continua** (S): que Jarvis mejore con el uso (germen ya en core/mark_ii.py — auto-mejora). ✅ core/evolution.py (Motor de Evolución, servicio #34): aprende de su PROPIA operación — assess_evolution cruza la telemetría de la coraza de tools (fiables/poco fiables/circuito abierto) y los errores reincidentes de error_kb (candidatos a auto-mejora Mark II) y saca lecciones priorizadas, que acumula deduplicadas en un diario (logs/evolution_journal.jsonl) evolucionando con el tiempo; assess/new_lessons/format puros. Voz "evoluciona" (extrae lecciones) y "¿qué has aprendido?" (recuento). Meta-capa que decide QUÉ mejorar; la ejecución es Mark II. 20 tests, 71% mutation.
