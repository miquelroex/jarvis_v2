...
* [~] **APIs de datos en vivo (legal)**: noticias, finanzas (bolsa/cripto), vuelos, tráfico, sismos, clima detallado — con webhooks/streams en lugar de consultas puntuales. *"Señor, la acción que vigilaba acaba de caer un 4%."* ✅ core/live_data.py: APIs públicas SIN clave — cripto (CoinGecko), sismos (USGS), tech-news (Hacker News); parsers puros + HTTP aislado; expuestas por voz ("precio del bitcoin", "últimos terremotos", "noticias de tecnología") y como fuente del Motor de Fusión con relevancia por tema (sólo consulta lo que la pregunta menciona). 27 tests, 100% mutation score. (Pendiente opcional: más APIs —bolsa/vuelos/tráfico— y webhooks/streams proactivos.)
